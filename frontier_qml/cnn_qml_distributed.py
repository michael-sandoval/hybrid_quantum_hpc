...
def train_model(rank, world_size): #model, criterion, optimizer, scheduler, num_epochs):
    ...
    model.fc = DressedQuantumNet(rank=rank)
    ...
    data_dir = '/path/to/data/hymenoptera_data'
...
def run_model(train_fn, world_size):
    mp.spawn(train_fn, nprocs=world_size, args=(world_size,))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-g', '--gpus', default=1, type=int, help='number of gpus to train with')
    ...
    print(f'Total GPUs being used this run: {args.gpus}')

    run_model(train_model, world_size)
